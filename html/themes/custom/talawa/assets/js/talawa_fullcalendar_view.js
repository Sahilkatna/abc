...
(function ($, Drupal) {
  Drupal.behaviors.talawafullcalendarView = {
    attach: function (context, settings) {
      $('#calendar').fullCalendar({
        ...
        eventRender: function (event, $el) {
          console.log($el);
          // var fcTime = $el.find('.fc-time');
          // var time = '';
          // if (time = fcTime.text()) {
          //   $(".fc-time", $el).prepend("Start Time ");
          // }

          // $el.find('.fc-title').insertBefore($el.find('.fc-time'));
          // $el.find('.fc-time').before("<br>");
          // Popup tooltip.
          ...
          if (event.ranges) {
            ...
            }).length) > 0; // If it isn't in one of the ranges, don't render
